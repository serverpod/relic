...
void main() {
  ...
  parameterizedGroup(
    descriptionBuilder: (v) =>
        'Given a "${v.key}" header when calling Headers.build',
    body: (v) {
      Headers.build((mh) {
        final header = v[mh];
        singleTest(
            description: 'then raw value not set',
            actual: mh,
            expected: isNot(contains(v.key)));
        singleTest(
            description: 'then not isSet',
            actual: header.isSet,
            expected: isFalse);
        singleTest(
            description: 'then set fails if type of value wrong',
            actual: () => header.set(Object()),
            expected: throwsA(isA<TypeError>()));
        singleTest(
            description: 'then set to null succeeds',
            actual: () => header.set(null),
            expected: returnsNormally);
        singleTest(
            description: 'then set succeeds if value is correct type',
  ...
  parameterizedGroup(
    descriptionBuilder: (v) => 'Given a "${v.key}" header with no raw value',
    body: (v) {
      ...
      singleTest(
          ...
          expected: isNot(contains(v.key)));
      singleTest(
          description: 'then isSet is false',
          actual: header.isSet,
          expected: isFalse);
      singleTest(
          description: 'then isValid is false',
          actual: header.isValid,
          expected: isFalse);
      singleTest(
          description: 'then valueOrNull is null',
          actual: header.valueOrNull,
          expected: isNull);
      singleTest(
          description: 'then valueOrNullIfInvalid is null',
          actual: header.valueOrNullIfInvalid,
          expected: isNull);
      singleTest(
          description: 'then value throws',
          actual: () => header.value,
          expected: throwsMissingHeader);
    },
    variants: Headers.all,
  );
  ...
  parameterizedGroup(
    descriptionBuilder: (v) =>
        'Given a "${v.key}" header with an empty raw value',
    body: (v) {
      ...
      singleTest(
          description: 'then raw value set',
          actual: headers,
          expected: contains(v.key));
      singleTest(
          description: 'then isSet is true',
          actual: header.isSet,
          expected: isTrue);
      singleTest(
          description: 'then isValid is false',
          ...
          expected: isFalse);
      singleTest(
          description: 'then valueOrNull throws',
          actual: () => header.valueOrNull,
          expected: throwsInvalidHeader);
      singleTest(
          description: 'then valueOrNullIfInvalid is null',
          actual: header.valueOrNullIfInvalid,
          expected: isNull);
  ...
  parameterizedGroup(
    descriptionBuilder: (v) =>
        'Given a "${v.key}" header with a raw value "[\'invalid\']"}',
    body: (v) {
  ...
  parameterizedTest(
      descriptionBuilder: (v) => 'Given a "${v.key.key}" header '
  ...
  parameterizedTest(
      ...
      variants: <HeaderFlyweight, dynamic Function(MutableHeaders)>{
        ...
        Headers.ifNoneMatch: (h) =>
            h.ifNoneMatch = IfNoneMatchHeader.wildcard(),
        Headers.ifRange: (h) =>
