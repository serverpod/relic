...
void main() {
  ...
  test('Given a request for a file in a non-existent directory, '
      'when the request is made, '
      'then a 404 Not Found status is returned', () async {
    final response = await makeRequest(
      handler,
      '/non_existent_dir/some_file.txt',
    );
  ...
  test('Given a request with path traversal attempt, '
      'when the request is made, '
      'then a 404 Not Found status is returned', () async {
  ...
  test('Given a request with URL encoded path traversal attempt, '
      'when the request is made, '
      'then a 404 Not Found status is returned', () async {
    final response = await makeRequest(
      handler,
