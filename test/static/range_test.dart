...
void main() {
  ...
  group('Given a single byte range request', () {
    test(
        ...
        () async {
      final headers = Headers.build((final mh) => mh.range =
          RangeHeader(ranges: [Range(start: 0, end: 4)])); // 0-4 (5 bytes)
    ...
    test(
        ...
        () async {
      final headers = Headers.build((final mh) => mh.range =
          RangeHeader(ranges: [Range(start: 10)])); // 10-END (6 bytes)
  ...
  group('Given a multiple byte range request', () {
    test(
      ...
      final headers = Headers.build((final mh) => mh.range = RangeHeader(
            ranges: [
      ...
      final boundary =
          _extractBoundary(response.headers[Headers.contentTypeHeader]!.first);
...
String? _extractBoundary(final String contentType) {
  ...
  final boundary =
      boundaryPart.trim().substring('boundary='.length).replaceAll('"', '');
