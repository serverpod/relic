...
4. Switch to Relic routing: Replace Router from shelf_router with `RelicApp`'s `get`, `post`, `put`, or `delete` methods. Replace `<id>` path parameters with `:id` and read them via `request.rawPathParameters[#id]`.
