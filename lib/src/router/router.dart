...
///
/// Supports static paths (e.g., `/users/profile`) and paths with named
/// parameters (e.g., `/users/:id`). Normalizes paths before matching.
final class Router<T> {
  ...
  // Helper to check if a normalized path contains parameter segments.
  ...
  ///
  /// The [path] string defines the route pattern. Segments starting with `:`
  /// (e.g., `:id`) are treated as parameters.
  /// The associated [value] (e.g., a request handler) is stored for this route.
  ///
  /// Routes are classified as static or dynamic based on whether the
  /// [path]'s normalized segments contain a parameter (like `:id`). Static routes are stored in a HashMap for
  /// O(1) average lookup, while dynamic routes are stored in a [PathTrie].
  ///
  /// If a static route with the same normalized path already exists, an
  /// [ArgumentError] is thrown.
  /// If adding a dynamic route path that already exists, the value is overwritten
  /// (and a warning is printed by the underlying PathTrie).
  ...
  ///
  /// The input [path] string is normalized before lookup.
  /// Static routes are checked first, followed by dynamic routes in the trie.
  ///
