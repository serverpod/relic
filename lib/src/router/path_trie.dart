...
class Match<T> {
  ...
  /// Creates a [Match] with the given [value] and [parameters].
  const Match(this.value, this.parameters, this.matched, this.remaining);
}
...
final class PathTrie<T> {
  ...
  ///
  /// Returns a [Match] containing the associated value and extracted
  /// parameters if a matching path is found, otherwise returns `null`.
  Match<T>? lookup(final NormalizedPath normalizedPath) {
    final segments = normalizedPath.segments;
    _TrieNode<T> currentNode = _root;
    ...
    return value != null
        ? Match(value, parameters, matchedPath, remainingPath)
        : null;
  }
