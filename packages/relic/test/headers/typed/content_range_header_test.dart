...
void main() {
  group('Given a Content-Range header with validation', () {
    ...
    test('when an invalid Content-Range header with start greater than end is '
        'passed then the server responds with a bad request including a message '
        'that states the header value has an invalid range', () async {
      expect(
        getServerRequestHeaders(
          server: server,
          touchHeaders: (final h) => h.contentRange,
    ...
    test('when a Content-Range header with an invalid value is passed '
