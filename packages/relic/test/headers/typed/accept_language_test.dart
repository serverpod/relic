...
void main() {
  group('Given an Accept-Language header with validation', () {
    ...
    test('when an Accept-Language header with empty language is passed then '
        'the server responds with a bad request including a message that '
        'states the language is invalid', () async {
      expect(
        getServerRequestHeaders(
          server: server,
          touchHeaders: (final h) => h.acceptLanguage,
    ...
    test('when an Accept-Language header with an invalid value is passed '
    ...
    test('when an Accept-Language header is passed then it should parse the '
        'language correctly', () async {
      final headers = await getServerRequestHeaders(
        server: server,
        touchHeaders: (final h) => h.acceptLanguage,
        headers: {'accept-language': 'en'},
      ...
      expect(
        headers.acceptLanguage?.languages.map((final e) => e.language).toList(),
        equals(['en']),
      );
    });

    test('when an Accept-Language header is passed without quality then the '
      ...
      expect(
        headers.acceptLanguage?.languages.map((final e) => e.quality).toList(),
        equals([1.0]),
      );
    });
    ...
    group('when multiple Accept-Language headers are passed', () {
      test('then they should parse the languages correctly', () async {
        final headers = await getServerRequestHeaders(
          server: server,
          touchHeaders: (final h) => h.acceptLanguage,
      ...
      test('then they should parse the qualities correctly', () async {
        final headers = await getServerRequestHeaders(
          server: server,
          touchHeaders: (final h) => h.acceptLanguage,
          headers: {'accept-language': 'en, fr, de'},
  ...
  group('Given an Accept-Language header without validation', () {
    late RelicServer server;
