...
void main() {
  group('Given an If-Match header with validation', () {
    ...
    test('when an empty If-Match header is passed then the server responds '
    ...
    test('when an If-Match header with an invalid value is passed '
    ...
    test('when an If-Match header with a single valid ETag is passed then it '
    ...
    test('when an If-Match header with a wildcard (*) is passed then it '
        'should parse correctly', () async {
      final headers = await getServerRequestHeaders(
        server: server,
        touchHeaders: (final h) => h.ifMatch,
    ...
    group('when multiple ETags are passed', () {
      test('ETags are passed then they should parse correctly', () async {
        final headers = await getServerRequestHeaders(
          server: server,
          touchHeaders: (final h) => h.ifMatch,
      ...
      test('with W/ weak validator prefix should be accepted', () async {
        ...
        expect(headers.ifMatch?.etags.every((final e) => e.isWeak), isTrue);
      });
  ...
  group('Given an If-Match header without validation', () {
    late RelicServer server;
