...
void main() {
  group('Given a Referer header with validation', () {
    ...
    test('when a Referer header with an empty value is passed then the server '
        'responds with a bad request including a message that states the '
        'header value cannot be empty', () async {
      expect(
    ...
    test('when a Referer header with an invalid URI is passed then the server '
    ...
    test('when a Referer header with an invalid port number is passed '
        'then the server responds with a bad request including a message that '
        'states the URI format is invalid', () async {
      expect(
        getServerRequestHeaders(
          server: server,
    ...
    test('when a Referer header with an invalid value is passed '
    ...
    test(
      ...
      () async {
        ...
        expect(headers.referer?.port, equals(8080));
      },
    );

    test('when a Referer header with extra whitespace is passed then it should '
        'parse the URI correctly', () async {
      final headers = await getServerRequestHeaders(
        server: server,
