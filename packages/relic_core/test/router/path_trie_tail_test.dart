...
void main() {
  group('PathTrie Tail (**) Matching', () {
    ...
    test('Given a trie with path /static/**, '
    ...
    group('Root path (/) matching with root tail (/**) interactions', () {
      ...
      test('Given a trie with path / and path /** defined, '
          'when the root path / is looked up, '
    ...
    test('Given a trie with /assets/js/app.js and /assets/**, '
    ...
    test('Given a trie with /foo/bar/** and /foo/**, '
        'when paths are looked up, '
    ...
    test('Given a trie with path /user/:id/files/**, '
      ...
      final result = trie.lookup(
        NormalizedPath('/user/42/files/docs/report.pdf'),
      );
      expect(result, isNotNull);
      expect(result!.value, 1);
    ...
    test('Given a trie with path /data/export/**, '
    ...
    test('Given an empty trie, '
      ...
      expect(
        () => trie.add(NormalizedPath('/downloads/**foo'), 1),
        throwsArgumentError,
    ...
    group('Tail and Other Segment interaction validation', () {
      test('Given a trie with /test/**, '
          'when adding /test/:id, '
          'then an ArgumentError is thrown', () {
        trie.add(NormalizedPath('/test/**'), 1);
        expect(
          () => trie.add(NormalizedPath('/test/:id'), 2),
          throwsArgumentError,
          reason: 'Parameter after tail at same level',
        );
      });

      test('Given a trie with /test/:id, '
          'when adding /test/**, '
          'then an ArgumentError is thrown', () {
        trie.add(NormalizedPath('/test/:id'), 1);
        expect(
          () => trie.add(NormalizedPath('/test/**'), 2),
          throwsArgumentError,
